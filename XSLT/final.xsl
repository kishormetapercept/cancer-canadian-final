...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="fig">
        <fig>
            <xsl:attribute name="id">
                <xsl:value-of select="child::image/@mediaid"/>
            </xsl:attribute>
            <xsl:choose>
    ...
    <xsl:template match="image">
        <image>
            ...
            <xsl:apply-templates select="@* except @mediaid"/>
    ...
    <xsl:template match="p[@outputclass][child::p]|div|span|br[preceding::*[1][self::br]]">
        <xsl:apply-templates/>
    </xsl:template>
    
    <xsl:template match="br[following-sibling::*[1][self::br] and not(preceding-sibling::*[1][self::br])]">
