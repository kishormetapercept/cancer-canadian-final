...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="item">
        ...
        <concept id="{concat('X_', substring-before(substring-after(replace(@id, '-', ''), '{'), '}'))}" xml:lang="{@language}" filename="{lower-case(concat($concat, '_', @language))}.dita" 
            ...
            <xsl:choose>
                ...
                <xsl:when test="descendant::field[@key='contactoptiontitle']">
                    <title outputclass="contactoptiontitle">
